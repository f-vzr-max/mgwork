...
export async function generateMetadata() {
  ...
  return {
    title: t("guides.entreprise.metaTitle"),
    description: t("guides.entreprise.metaDescription"),
  };
}

const STEP_KEYS = ["s1", "s2", "s3", "s4", "s5", "s6"] as const;

export default async function GuideEntreprisePage() {
  const t = await getTranslations("marketing");
  return (
    <PublicShell active={null}>
      <Section padY={80}>
        <SectionHeader
          eyebrow={t("guides.entreprise.eyebrow")}
          title={t("guides.entreprise.title")}
          subtitle={t("guides.entreprise.subtitle")}
          align="left"
        />
        <Stack gap={16} style={{ marginTop: 32, maxWidth: 760 }}>
              ...
              <Stack dir="row" gap={20} align="flex-start">
                ...
                <div style={{ flex: 1 }}>
                  <h3 className="mg-h4" style={{ margin: "4px 0 8px" }}>
                    {t(`guides.entreprise.${k}.title`)}
                  </h3>
                  <p
                    className="mg-body"
                    style={{ margin: 0, color: "hsl(var(--muted-foreground))" }}
                  >
                    {t(`guides.entreprise.${k}.body`)}
                  </p>
                </div>
              </Stack>
            </Card>
          ...
          <div style={{ marginTop: 16 }}>
            <Link href="/sign-up" style={{ textDecoration: "none" }}>
              <Button>{t("guides.entreprise.cta")}</Button>
            </Link>
          </div>
        </Stack>
      </Section>
