...
function formatActionLabel(action: string): string {
  ...
  if (verb === "upload") return "Uploaded";
...
export default async function StaffDocumentReviewPage({ params }: { params: { id: string } }) {
  ...
  return (
    <>
      <PageHeader title={`Review: ${doc.type.replace(/_/g, " ")}`} description={`${ownerKind} · ${ownerLabel}`}>
        <Button asChild variant="outline" size="sm">
          <Link href="/staff/documents">Back to queue</Link>
        </Button>
      </PageHeader>
        ...
        <div className="space-y-4">
            ...
            <CardContent>
              {isPending ? (
                ...
                <p className="text-sm text-muted-foreground">
                  This document is no longer pending. Status: {doc.status.toLowerCase()}.
                </p>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="p-0">
              {auditRows.length === 0 ? (
                <p className="px-6 py-4 text-sm text-muted-foreground">No prior actions.</p>
              ) : (
                <ul className="divide-y">
                  {auditRows.map((row) => (
                    <li key={row.id} className="flex items-start justify-between gap-4 px-6 py-3 text-sm">
                      <div>
                        <div className="font-medium">{formatActionLabel(row.action)}</div>
